...
以下是 `cmd/ckjr-cli/config/routes/agent.yaml` 中 create 路由的完整定义：
...
1. 在 `cmd/ckjr-cli/config/routes/` 目录下创建新的 YAML 文件（如 `example.yaml`）
2. 本地编译并测试（见下方编译验证）
3. `name` 不能与已有资源重名
...
ckjr-cli route import \
  ...
  --file cmd/ckjr-cli/config/routes/example.yaml \
  --name-desc "示例资源管理"
```
...
echo 'curl -X POST https://api.example.com/admin/example/list -d '"'"'{"page":1}'"'"'' \
  | ckjr-cli route import --file cmd/ckjr-cli/config/routes/example.yaml --name-desc "示例"
```
