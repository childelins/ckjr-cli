...
以下是 `cmd/routes/agent.yaml` 中 create 路由的完整定义：
...
1. 在 `cmd/routes/` 目录下创建新的 YAML 文件（如 `example.yaml`）
2. 本地编译并测试（见下方编译验证）
3. `name` 不能与已有资源重名
...
echo 'curl -X POST https://api.example.com/admin/example/list -d '"'"'{"page":1}'"'"'' \
  | ckjr-cli route import --file cmd/routes/example.yaml --name-desc "示例"
```
